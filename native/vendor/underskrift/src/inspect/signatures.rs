...
pub struct PdfSignatureInspection {
    ...
    /// Object number referenced by the catalog's `/Perms /DocMDP`, if present.
    pub catalog_doc_mdp_obj_num: Option<u32>,
...
pub fn inspect_signatures(pdf_data: &[u8]) -> Result<PdfSignatureInspection, InspectError> {
    ...
    let catalog_doc_mdp_obj_num = catalog
        .get(b"Perms")
        .ok()
