...
impl IncrementalWriter {
    ...
    /// Write a regular incremental update that does not introduce a signature
    /// placeholder. This is used for DSS/LTV revisions, which must preserve all
    /// existing signature byte ranges while appending validation material.
    pub fn write_update(self) -> Result<Vec<u8>, CoreError> {
        if self.sig_dict_id.is_some() {
            return Err(CoreError::InvalidStructure(
                "a regular update cannot contain a tracked signature dictionary".into(),
            ));
        }
        let mut buf = self.original.clone();
        if buf.last() != Some(&b'\n') {
        ...
        if xref_entries.is_empty() {
            return Err(CoreError::InvalidStructure(
                "incremental update has no objects".into(),
            ));
        }
        if self.use_xref_stream {
            self.write_xref_stream_section(&mut buf, &xref_entries)?;
