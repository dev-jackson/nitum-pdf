...
pub fn prepare_doc_timestamp(
    ...
    // PDF field names are identifiers, not merely labels. Reusing the default
    // name for a later archive timestamp makes strict validators resolve two
    // revisions through the same field and report that the signed dictionary
    // changed. Allocate the next suffix when the requested name already exists.
    let field_name = unique_timestamp_field_name(&doc, &options.field_name)?;
...
fn unique_timestamp_field_name(doc: &Document, requested: &str) -> Result<String, PdfSignError> {
    let existing = parser::extract_signatures(doc)?
        .into_iter()
        .map(|signature| signature.field_name)
        .collect::<std::collections::HashSet<_>>();
    if !existing.contains(requested) {
        return Ok(requested.to_owned());
    }

    let stem = requested.trim_end_matches(|character: char| character.is_ascii_digit());
    ...
    (1_u32..)
        ...
        .ok_or_else(|| {
            PdfSignError::Core(CoreError::InvalidStructure(
                "no se pudo asignar un nombre único al sello de tiempo".into(),
            ))
        })
}
...
mod tests {
    ...
    #[test]
    fn repeated_default_timestamp_name_is_made_unique() {
        let pdf_data = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/fixtures/sample.pdf"
        ));
        ...
        let (first, _) = prepare_doc_timestamp(pdf_data, &options).unwrap();
        let (second, _) = prepare_doc_timestamp(&first, &options).unwrap();
        let parsed = Document::load_mem(&second).unwrap();
        let names = parser::extract_signatures(&parsed)
            .unwrap()
            .into_iter()
            .map(|signature| signature.field_name)
            .collect::<Vec<_>>();

        assert!(names.contains(&"DocTimeStamp1".to_owned()));
        assert!(names.contains(&"DocTimeStamp2".to_owned()));
    }
}
