...
/// Extract the signer certificate chain embedded in a CMS `SignedData` value.
///
/// This is useful when building a PDF DSS: validation material is required for
/// RFC 3161 document timestamps as well as for the approval signature itself.
pub fn extract_cms_signer_chain(cms_bytes: &[u8]) -> Result<Vec<Certificate>, VerifyError> {
    let content_info = ContentInfo::from_der(cms_bytes).map_err(|e| {
        VerifyError::CmsVerification(format!("failed to parse CMS ContentInfo: {e}"))
    })?;
    if content_info.content_type != rfc5911::ID_SIGNED_DATA {
        return Err(VerifyError::CmsVerification(
            "CMS content is not SignedData".to_owned(),
        ));
    }
    let signed_data_der = content_info.content.to_der().map_err(|e| {
        VerifyError::CmsVerification(format!("failed to encode CMS SignedData: {e}"))
    })?;
    let signed_data = SignedData::from_der(&signed_data_der).map_err(|e| {
        VerifyError::CmsVerification(format!("failed to parse CMS SignedData: {e}"))
    })?;
    let certificates = extract_certificates(&signed_data);
    let signer_info =
        signed_data.signer_infos.0.iter().next().ok_or_else(|| {
            VerifyError::CmsVerification("CMS SignedData has no signer".to_owned())
        })?;
    let signer = find_signer_certificate(signer_info, &certificates).ok_or_else(|| {
        VerifyError::CmsVerification("CMS signer certificate was not embedded".to_owned())
    })?;

    super::chain_verify::build_chain(&signer, &certificates).map_err(|error| {
        VerifyError::CmsVerification(format!("failed to build CMS signer chain: {error}"))
    })
}
...
fn verify_cms_signature(
    ...
    if *sig_alg_oid == db::rfc5912::RSA_ENCRYPTION {
        if *digest_alg_oid == db::rfc5912::ID_SHA_256 {
            verify_rsa_cms::<sha2::Sha256>(data, signature, spki_der)
        } else if *digest_alg_oid == db::rfc5912::ID_SHA_384 {
            verify_rsa_cms::<sha2::Sha384>(data, signature, spki_der)
        } else if *digest_alg_oid == db::rfc5912::ID_SHA_512 {
