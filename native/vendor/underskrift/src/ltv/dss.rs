...
use crate::core::{incremental::IncrementalWriter, parser::extract_metadata};
...
/// Append a DSS as a signature-preserving incremental PDF revision.
pub fn add_document_security_store(
    pdf_data: &[u8],
    builder: &DssBuilder,
) -> Result<Vec<u8>, LtvError> {
    if builder.is_empty() {
        return Err(LtvError::Dss("refusing to append an empty DSS".into()));
    }
    let mut doc = Document::load_mem(pdf_data)
        .map_err(|error| LtvError::Dss(format!("failed to parse PDF: {error}")))?;
    let metadata = extract_metadata(&doc)
        .map_err(|error| LtvError::Dss(format!("failed to read PDF metadata: {error}")))?;
    let original_max_id = metadata.max_id;
    let dss = builder.build_dss_dict(&mut doc)?;
    let dss_id = doc.add_object(dss);
    doc.get_dictionary_mut(metadata.root_id)
        .map_err(|error| LtvError::Dss(format!("failed to update catalog: {error}")))?
        .set("DSS", Object::Reference(dss_id));

    let mut writer = IncrementalWriter::new(
        pdf_data.to_vec(),
        metadata.trailer_size,
        metadata.xref_offset,
        metadata.root_id,
        0,
    );
    writer.set_trailer_meta(metadata.id, metadata.encrypt, metadata.uses_xref_stream);
    let mut object_ids = doc
        .objects
        .keys()
        .copied()
        .filter(|id| id.0 > original_max_id)
        .collect::<Vec<_>>();
    object_ids.sort_unstable();
    for id in object_ids {
        if let Ok(object) = doc.get_object(id) {
            writer.add_object(id, object.clone());
        }
    }
    let catalog = doc
        .get_object(metadata.root_id)
        .map_err(|error| LtvError::Dss(format!("failed to read updated catalog: {error}")))?;
    writer.add_object(metadata.root_id, catalog.clone());
    writer
        .write_update()
        .map_err(|error| LtvError::Dss(format!("failed to write DSS revision: {error}")))
}
